    ...
    window.addEventListener('load', () => {
        ...
        if (rawPath) {
            const targetElement = document.getElementById(rawPath);
            if (targetElement) {
                setTimeout(() => {
    ...
    document.querySelectorAll('a[href^="/"], a[href^="#"]').forEach(link => {
        link.addEventListener('click', function (e) {
            ...
            if (targetPath === '/' || targetPath === '#top' || targetPath === '#') {
                e.preventDefault();
            ...
            const sectionId = targetPath.replace('/', '').replace('#', '').toLowerCase();
            ...
            if (targetElement) {
                ...
                // Push clean URL format (e.g., /About) to address bar
                const cleanFormattedPath = '/' + sectionId.charAt(0).toUpperCase() + sectionId.slice(1);
                history.pushState(null, '', cleanFormattedPath);
            }
        });
    });
